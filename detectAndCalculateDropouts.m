...
function [dropoutStartingPoints, dropoutEndingPoints, totalDropouts, totalDropoutTime, dropoutRatio] = ...
    ...
    for k=1:length(dropoutIndices)
        ...
        searchThreshold = 0.6;
        [inddc1,indds1]=find(dropoutStartingPoints>(dpStartPoint-searchThreshold) & dropoutStartingPoints<(dpStartPoint+searchThreshold) & dropoutStartingPoints~=0); %Let's find the positions of similar values
        [inddc2,indds2]=find(dropoutEndingPoints>(dpEndPoint-searchThreshold) & dropoutEndingPoints<(dpEndPoint+searchThreshold)& dropoutEndingPoints~=0); %Let's find the positions of similar values
